...
from .utils import send_password_reset_mail, send_verification_email
...
def password_reset_view(request):
    if request.method == 'POST':
        ...
        try:
            ...
            send_password_reset_mail(username=user.username, email=user.email, link=link)
...
@api_view(['GET'])
@permission_classes([IsVerified])
def get_specific_user_view(request, id:str): 
    if request.method == 'GET':
        current_user = request.user # user logged in
...
@api_view(['PUT', 'PATCH'])
@permission_classes([IsVerified])
def update_user_info_view(request, id:str):
...
@api_view(['DELETE'])
@permission_classes([IsVerified])
def delete_user_view(request, id:str):
    if request.method == 'DELETE':
        ...
        if not current_user.is_staff:
            return Response (
                {
                    'success':True,
                    'message':'You are not an admin. Hence, you can not perform this action'
                }, status=status.HTTP_403_FORBIDDEN
            )
...
def search_users_view(request):
    if request.method == 'GET':
        ...
        return Response (
            {
                'success':True,
                'message':'Search results are below;',
                'user(s)':serializer.data
            }, status=status.HTTP_200_OK
        )